...
const CallWindow = ({ appointment, isCaller, onClose }) => {
    ...
    const [isAudioOnly, setIsAudioOnly] = useState(false) // Default to video call
    ...
    useEffect(() => {
        ...
        const initCall = async () => {
            try {
                ...
                // Create Peer instance with patient ID
                const peer = new Peer(userData._id, {
                    host: '/',
                    port: '',
                    path: '',
                    secure: true,
                ...
                peer.on('open', (id) => {
                    ...
                    if (isCaller) {
                        // If caller, send signal to doctor
                        ...
                        // If receiver (already accepted), signal doctor that we are ready
        ...
        const handleCallAccepted = () => {
            console.log('Call accepted by doctor, initiating PeerJS call')
            setCallStatus('Connecting...')
            if (peerRef.current && streamInstance) {
                // Call the doctor (doctor's peer ID is their doctor ID / docId)
                const call = peerRef.current.call(appointment.docId, streamInstance)
